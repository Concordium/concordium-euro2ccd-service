...
const CREATE_TABLES: &str = "CREATE TABLE IF NOT EXISTS read_values (value DOUBLE NOT NULL, \
                             timestamp DATETIME NOT NULL, label VARCHAR(15)); CREATE TABLE IF NOT EXISTS updates \
                             (numerator BIGINT UNSIGNED NOT NULL, denominator BIGINT UNSIGNED NOT \
                             NULL, timestamp DATETIME NOT NULL);";

const CHECK_FOR_LABEL: &str = "SELECT count(*) FROM information_schema.columns WHERE table_name = 'read_values' AND column_name = 'label' and table_schema = DATABASE();";
// When we add the label column, it is assumed that all values are from v1, so we label them: bitfinex(v1)
...
pub fn create_tables(conn: &mut PooledConn) -> anyhow::Result<()> {
    ...
    // The check for label should return 1/0 depending on the existance of the label column
    match conn.query_first(CHECK_FOR_LABEL)? {
        Some(0) => Ok(conn.query_drop(format!("ALTER TABLE read_values ADD COLUMN label VARCHAR(15) DEFAULT '{}';", DEFAULT_LABEL))?),
        Some(_) => Ok(()),
